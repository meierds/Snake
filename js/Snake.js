...
const Game = {
  ...
  lose() {
    ...
    ctx.fillText(
      "Play Again? Press <Space> to replay",
      size * (Game.grid.xMax / 2),
      size * (Game.grid.yMax / 2)
    );
  },
  replay() {
    ...
    Snake.body.location = [
      new Point(Snake.head.location.x, Snake.head.location.y)
    ];
    Snake.body.dir = [];
    this.eventStack = [];
    this.framecount = 0;
  ...
  win() {
    ...
    this.won = true;
    console.log(
      "Woah. You actually won... I wish I had something better to give you, but congratulations!"
    );
  },
...
var Snake = {
  ...
  head: {
    color: "red",
    location: new Point(
      Math.floor(Game.grid.xMax / 2),
      Math.floor(Game.grid.yMax / 2)
    ),
    dir: [0, 0],
    grid: [],
    foodEaten: false,
    ...
    checkCollision(dir) {
      ...
      if (
        testX >= Game.grid.xMax ||
        testX < 0 ||
        testY >= Game.grid.yMax ||
        testY < 0
      )
        collided = true;
  ...
  body: {
    color: "black",
    location: [
      new Point(Math.floor(Game.grid.xMax / 2), Math.floor(Game.grid.yMax / 2))
    ],
    dir: [[0, 0]],
    move() {
      this.dir.pop();
  ...
  draw() {
    if (this.body.dir.length > 0) {
      ...
      for (let i = 1; i < Snake.body.location.length; i++) {
        ctx.rect(
          Snake.body.location[i].xactual,
          Snake.body.location[i].yactual,
          this.size,
          this.size
        );
      }
    ...
    ctx.beginPath();
    ctx.rect(
      this.head.location.xactual,
      this.head.location.yactual,
      this.size,
      this.size
    );
...
var Food = {
  location: new Point(
    Math.floor(Math.random() * Game.grid.xMax),
    Math.floor(Math.random() * Game.grid.yMax)
  ),
  size: Snake.size,
  ...
  draw() {
    ctx.beginPath();
    ctx.arc(
      this.location.xactual + (this.size + 1) / 2,
      this.location.yactual + (this.size + 1) / 2,
      this.size / 2,
      0,
      2 * Math.PI
    );
